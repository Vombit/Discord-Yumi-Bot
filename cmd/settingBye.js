...
module.exports.run = async (bot,message,args) => {
    ...
    var yes = "y";
    message.channel.send(`${lang.settings.set.yornbye}: y/n`)
    ...
    sett.on('collect', (message) => {
        ...
        if (message.content == yes){
            message.channel.send(`${lang.settings.set.setbye}`);
            ...
            logs.on('collect', (message) => {
                logs.stop();
                base[message.guild.id].msgBye = message.content;
...
module.exports.help = {
    name: "setbye",
    aliases: [""],
    description: "Установка сообщения при выходе пользователя",
    usage: ".settings",
    noalias: "No Aliases",
    accessible: "",
    enabled: true
